...
KIND_CLUSTER ?= pihole-ha-operator-test-e2e
...
test-e2e: setup-test-e2e manifests generate fmt vet ## Run the e2e tests. Expected an isolated environment using Kind.
	...
	CERT_MANAGER_INSTALL_SKIP=true \
	go test -timeout=30m -failfast -tags=e2e ./test/e2e/ -v -ginkgo.v \
	-ldflags="$(LDFLAGS)"
...
docker-buildx-operator: ## Build and push docker image for the manager for cross-platform support
	...
	$(CONTAINER_TOOL) buildx use pihole-ha-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm pihole-ha-operator-builder
	rm Dockerfile.cross
...
HELM_NAMESPACE ?= pihole-ha-operator-system
