...
var _ = Describe("PiHoleCluster Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			...
			if err != nil && errors.IsNotFound(err) {
				resource := createMinimalCluster(typeNamespacedName, piholeImage)
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance PiHoleCluster")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &PiHoleClusterReconciler{
				Client: k8sClient,
